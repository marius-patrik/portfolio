...
import RateCard from '../../components/cards/card-rate';
...
const IndexPage: react.FC = () => {
  const { data, error, isLoading } = useSWR<RateItem[]>('/api/rates', fetcher);
    ...
    return (
      <Body>
        <div className="text-red-500">
          ...
          <div>{error?.message || error?.error || JSON.stringify(error)}</div>
        </div>
      </Body>
    );
  }
    ...
    <Body pageTitle="Rates">
      ...
      >
        <div className="grid grid-cols-[1fr_5fr_1fr] gap-2">
          ...
          <div className="style-title mr-auto">Country</div>
